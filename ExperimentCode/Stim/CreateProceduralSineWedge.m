function [gratingid, gratingrect] = CreateProceduralSineWedge(windowPtr, width, height, backgroundColorOffset, radius, anglestart, angleend, orientation, contrastPreMultiplicator)
...
if nargin < 9 || isempty(contrastPreMultiplicator)
...
if isinf(radius)
    ...
    gratingShader = LoadGLSLProgramFromFiles(fullfile(currentFolder,'BasicSineWedgeShader'), debuglevel);
else
    % Load grating shader with circular aperture support:
    gratingShader = LoadGLSLProgramFromFiles({fullfile(currentFolder,'BasicSineWedgeShader.vert.txt'), fullfile(currentFolder,'ApertureSineWedgeShader.frag.txt')}, debuglevel);
    %gratingShader = LoadGLSLProgramFromFiles({'BasicSineWedgeShader.vert.txt', 'ApertureSineWedgeShader.frag.txt'}, debuglevel);
end
...
if ~isinf(radius)
    % Set radius of circular aperture:
    glUniform1f(glGetUniformLocation(gratingShader, 'AngleStart'), anglestart);
end

if ~isinf(radius)
    % Set radius of circular aperture:
    glUniform1f(glGetUniformLocation(gratingShader, 'AngleEnd'), angleend);
end


%glUniform1f(glGetUniformLocation(gratingShader, 'Orientation'), orientation);
