% look at 1 subject
% allvoxelsBOLDpa(ci,pa,ri,1:length(finalIdx),si)
...
bidsDir = '/Volumes/Vision/UsersShare/Rania/Project_dg/data_bids/'; %'/Volumes/server/Projects/Project_dg/data_bids'; 
load(fullfile(bidsDir, 'derivatives', 'dgGLM', 'hRF_glmsingle', 'allvoxelsBOLDpa.mat'))
load(fullfile(bidsDir, 'derivatives', 'dgGLM', 'hRF_glmsingle', 'allparamsBOLDpa.mat'))
...
for ri=1:length(rois)
    ...
    for di=1:length(allArray) % 8 directions
        ...
        % subjects (polar angles 1=verticle, 2, 3, 4, 5, 6, 7, 8 goes counterclockwise)
...
%     title(rois{roiNum}, 'FontSize',20)
    ...
    Zq = griddata(x, y, intensity, Xq, Yq, 'cubic');
