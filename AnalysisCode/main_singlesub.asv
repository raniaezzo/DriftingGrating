...
ses = 'ses-01'; %'ses-nyu3t02'; %'ses-nyu3t02'; %'ses-01';
space = 'fsnative';
...
surfSize = get_surfsize(subj); 

roiName = 'pMT_REmanual'; % %; 'V1_REmanual'; %  % 'hMTcomplex_REmanual'; %

primaryROIvertices = getROIidxs(subj, roiName, surfSize); % the name of Middle Temporal Cortex ROI
surfaceROI = nan(sum(surfSize),1);
...
%% Plot trial triggered average

ttave(matrices,datafiles, surfaceROI, tr_s, stimlength)


%% Consider changing betas to PSC 

% divide the beta weight by the mean signal for each voxel (across
% timepoints)

%checking = (results.allevents.modelmd{1,2}./results.allevents.meanvol) .* 100;
