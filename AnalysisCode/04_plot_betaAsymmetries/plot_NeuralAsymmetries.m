...
hRF_setting = 'glmsingle'; %'canonical' %
...
for ci=1:numel(isradial)
    ...
    if strcmp(projectName, 'dg') && ~radialvstang
        plotSepDirs(medianBOLDpa, 'mainCardinalVsMainOblique', 'Abs', projectSettings, radialvstang)
    elseif (strcmp(projectName, 'da') || strcmp(projectName, 'dots')) && ~radialvstang
        plotSepDirs(medianBOLDpa, 'mainCardinalVsMainOblique', 'Rel', projectSettings, radialvstang)
    end
...
for ci=1:numel(n_derivedConditions)
    ...
    % plot each absolute directions
    if (strcmp(projectName, 'da') || strcmp(projectName, 'dots')) && ~radialvstang
        newMatrix = compute_derivativeEachAbsDirection(medianBOLDpa, projectSettings);
        plotSepDirs(newMatrix, 'derivedCardinalVsDerivedOblique', 'Abs', projectSettings, radialvstang)
    elseif strcmp(projectName, 'dg') && ~radialvstang
        newMatrix = compute_derivativeEachRelDirection(medianBOLDpa, projectSettings);
        plotSepDirs(newMatrix, 'derivedCardinalVsDerivedOblique', 'Rel', projectSettings, radialvstang)
    end
