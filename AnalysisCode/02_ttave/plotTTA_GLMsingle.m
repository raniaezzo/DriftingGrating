function dataRange = plotTTA_GLMsingle(projectName, roiName, targetWedgeAngle, varargin)
...
p.addParameter('yLimOverride', [], @(x) isempty(x) || (isnumeric(x) && numel(x)==2));
p.parse(varargin{:});
opt = p.Results;
...
for cc = 1:4
    ...
    obsVals = squeeze(groupTTA_obs(targetW,cc,:))';
    predVals = squeeze(groupTTA_pred(targetW,cc,:))';
    allVals = [allVals, obsVals, predVals]; %#ok<AGROW>
...
% y-axis limits: use the caller-supplied shared range if given, otherwise
% this plot's own data range (with the same 10% padding convention used
% throughout this pipeline) -- returned as dataRange either way, so a
% caller can run once per experiment, take the union of both dataRanges,
% and re-call with 'yLimOverride' to get identical y-axes across plots.
allValsFinite = allVals(isfinite(allVals));
padY = 0.1 * (max(allValsFinite) - min(allValsFinite) + eps);
dataRange = [min(allValsFinite)-padY, max(allValsFinite)+padY];
if ~isempty(opt.yLimOverride)
    ylim(opt.yLimOverride);
else
    ylim(dataRange);
end
...
axesWidth_cm = (15.37/2)*1.25; axesHeight_cm = 3.37*2; % width halved then +25%, height doubled per request
